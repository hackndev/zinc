...
impl Pin {
  ...
  pub fn new(port: Port, pin_index: u8, mode: Mode, pull_type: PullType) -> Pin {
    use hal::stm32f4::peripheral_clock::PeripheralClock as clock;
    use self::Mode::*;
    let (reg, clock) = match port {
      PortA => (&reg::GPIOA, clock::GPIOAClock),
      PortB => (&reg::GPIOB, clock::GPIOBClock),
      PortC => (&reg::GPIOC, clock::GPIOCClock),
      PortD => (&reg::GPIOD, clock::GPIODClock),
      PortE => (&reg::GPIOE, clock::GPIOEClock),
      PortF => (&reg::GPIOF, clock::GPIOFClock),
      PortG => (&reg::GPIOG, clock::GPIOGClock),
      PortH => (&reg::GPIOH, clock::GPIOHClock),
      PortI => (&reg::GPIOI, clock::GPIOIClock),
    };
...
mod reg {
  ...
  extern {
    ...
    #[link_name="stm32f4_iomem_GPIOH"] pub static GPIOI: GPIO;
  }
}
